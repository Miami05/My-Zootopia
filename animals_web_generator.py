...
def human_join(items):
    """Join list items with commas and 'and' (e.g., 'A, B, and C')."""
    if not items:
        return ""
    if len(items) == 1:
        return str(items[0])
...
def make_animals_text(animals):
    """Generate HTML list items for each animal with their characteristics."""
...
def main():
    """Load animal data, generate HTML from template, and save output file."""
